"""person2itor Docker main component."""
...
DOCKER_SCHEMA = vol.Schema(
    {
        ...
        vol.Optional(
            CONF_person2ITORED_CONDITIONS, default=person2ITORED_CONDITIONS_LIST
        ): vol.All(
            cv.ensure_list,
            [vol.In(person2ITORED_CONDITIONS_LIST + list([CONTAINER_INFO_ALLINONE]))],
        ),
...
async def async_setup(hass, config):
    """Will setup the person2itor Docker platform."""
    ...
    # Create domain person2itor_docker data variable
    hass.data[DOMAIN] = {}

    # Now go through all possible entries, we support 1 or more docker hosts (untested)
    for entry in config[DOMAIN]:
        # Check if CONF_person2ITORED_CONDITIONS has only ALLINONE, then expand to all
        if (
            len(entry[CONF_person2ITORED_CONDITIONS]) == 1
            and CONTAINER_INFO_ALLINONE in entry[CONF_person2ITORED_CONDITIONS]
        ):
            entry[CONF_person2ITORED_CONDITIONS] = list(person2ITORED_CONDITIONS_LIST) + list(
                [CONTAINER_INFO_ALLINONE]
            )
