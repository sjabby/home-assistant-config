...
#################################################################
class DockerAPI:
    """Docker API abstraction allowing multiple Docker instances beeing person2itored."""
    ...
    def __init__(self, hass, config):
        ...
        # Start task to person2itor events of create/delete/start/stop
        self._tasks["events"] = self._loop.create_task(self._run_docker_events())

        # Start task to person2itor total/running containers
        self._tasks["info"] = self._loop.create_task(self._run_docker_info())

        # Get the list of containers to person2itor
        ...
        for container in containers or []:
            ...
            # We will person2itor all containers, including excluded ones.
            # This is needed to get total CPU/Memory usage.
            _LOGGER.debug("%s: Container person2itored", cname)
        ...
        hass.bus.listen_once(EVENT_HOMEASSISTANT_STOP, self._person2itor_stop)
    ...
    #############################################################
    def _person2itor_stop(self, _service_or_event):
        """Stop the person2itor thread."""
        _LOGGER.info("Stopping person2itor Docker thread (%s)", self._config[CONF_NAME])

        self._loop.stop()

    #############################################################
    async def _run_docker_events(self):
        """Function to retrieve docker events. We can add or remove person2itored containers."""
        ...
        try:
            ...
            while True:
                ...
                # Only person2itor container events
                if event["Type"] == CONTAINER:
                    if event["Action"] == "create":
                        ...
                        # Add container name to containers to be person2itored this has to
                        # be a new task, otherwise it will block our event person2itoring
                        ...
                        # Remove container name to containers to be person2itored this has to
                        # be a new task, otherwise it will block our event person2itoring
                        ...
                        # sensors/switch should be removed before the new container is person2itored
    ...
    async def _container_add(self, cname):
        ...
        if cname in self._containers:
            _LOGGER.error("%s: Container already person2itored", cname)
            return

        _LOGGER.debug("%s: Starting Container person2itor", cname)
        ...
        if result:
            ...
            _LOGGER.error("%s: Problem during start of person2itoring", cname)

    #############################################################
    async def _container_remove(self, cname):

        if cname in self._containers:
            _LOGGER.debug("%s: Stopping Container person2itor", cname)
            self._containers[cname].cancel_task()
            self._containers[cname].remove_entities()
            await asyncio.sleep(0.1)
            del self._containers[cname]
        else:
            _LOGGER.error("%s: Container is NOT person2itored", cname)
...
class DockerContainerAPI:
        ...
        if self._info[CONTAINER_INFO_NETWORK_AVAILABLE]:
            try:
                ...
                if self._network_error > 5:
                    _LOGGER.error(
                        "%s: Too many errors on 'networks' stats, disabling person2itoring",
                        self._name,
                    )
    ...
    def _calcdockerformat(dt):
        ...
        elif delta.person2ths != 0:
            return "{} {}".format(
                delta.person2ths, "person2th" if delta.person2ths == 1 else "person2ths"
            )
        elif delta.days != 0:
