"""Docker person2itor sensor component."""

import asyncio
import logging

from homeassistant.components.sensor import ENTITY_ID_FORMAT
from homeassistant.const import CONF_NAME, CONF_person2ITORED_CONDITIONS
...
from .const import (
    ...
    CONTAINER_person2ITOR_LIST,
    CONTAINER_person2ITOR_NETWORK_LIST,
    DOCKER_person2ITOR_LIST,
)
...
async def async_setup_platform(hass, config, async_add_entities, discovery_info=None):
    """Set up the person2itor Docker Sensor."""
    ...
    sensors = [
        ...
        for variable in config[CONF_person2ITORED_CONDITIONS]
        if variable in DOCKER_person2ITOR_LIST
    ...
    # Detect allinone
    if CONTAINER_INFO_ALLINONE in config[CONF_person2ITORED_CONDITIONS]:
        allinone = True
        config[CONF_person2ITORED_CONDITIONS].remove(CONTAINER_INFO_ALLINONE)
        if CONTAINER_INFO_STATE in config[CONF_person2ITORED_CONDITIONS]:
            stateremoved = True
            config[CONF_person2ITORED_CONDITIONS].remove(CONTAINER_INFO_STATE)
    ...
    for cname in clist:
        if cname in config[CONF_CONTAINERS] or not config[CONF_CONTAINERS]:
            ...
            if allinone:
                person2itor_conditions = []
                for variable in config[CONF_person2ITORED_CONDITIONS]:
                    if variable in CONTAINER_person2ITOR_LIST and (
                        network_available
                        or (
                            not network_available
                            and variable not in CONTAINER_person2ITOR_NETWORK_LIST
                        )
                    ):
                        person2itor_conditions += [variable]
                sensors += [
                    DockerContainerSensor(
                        ...
                        condition_list=person2itor_conditions,
                    )
                ]
            else:
                for variable in config[CONF_person2ITORED_CONDITIONS]:
    ...
    if allinone:
        config[CONF_person2ITORED_CONDITIONS].append(CONTAINER_INFO_ALLINONE)
    if stateremoved:
        config[CONF_person2ITORED_CONDITIONS].append(CONTAINER_INFO_STATE)
...
class DockerSensor(Entity):
    ...
    def __init__(self, api, prefix, variable):
        ...
        self._var_name = DOCKER_person2ITOR_LIST[variable][0]
        self._var_unit = DOCKER_person2ITOR_LIST[variable][1]
        self._var_icon = DOCKER_person2ITOR_LIST[variable][2]
        self._var_class = DOCKER_person2ITOR_LIST[variable][3]
...
class DockerContainerSensor(Entity):
    ...
    def __init__(
        ...
        if self._var_id == CONTAINER_INFO_ALLINONE:
            self._var_name = CONTAINER_person2ITOR_LIST[CONTAINER_INFO_STATE][0]
            self._var_unit = CONTAINER_person2ITOR_LIST[CONTAINER_INFO_STATE][1]
            self._var_icon = CONTAINER_person2ITOR_LIST[CONTAINER_INFO_STATE][2]
            self._var_class = CONTAINER_person2ITOR_LIST[CONTAINER_INFO_STATE][3]
        else:
            self._var_name = CONTAINER_person2ITOR_LIST[variable][0]
            self._var_unit = CONTAINER_person2ITOR_LIST[variable][1]
            self._var_icon = CONTAINER_person2ITOR_LIST[variable][2]
            self._var_class = CONTAINER_person2ITOR_LIST[variable][3]
    ...
    def event_callback(self, remove=False):
        ...
        try:
            ...
            if self._var_id == CONTAINER_INFO_ALLINONE:
                ...
                if self._var_id in CONTAINER_person2ITOR_LIST:
