...
from custom_components.hacs.hacsbase.backup import Backup, BackupNetDaeperson2
...
async def install_repository(repository):
    ...
    if repository.status.installed and repository.data.category == "netdaeperson2":
        persistent_directory = BackupNetDaeperson2(repository)
    ...
    if repository.validate.success:
        if repository.data.full_name not in repository.hacs.comperson2.installed:
            if repository.data.full_name == "hacs/integration":
                repository.hacs.comperson2.installed.append(repository.data.full_name)
...
async def reload_after_install(repository):
    ...
    if repository.data.category == "integration":
        ...
        try:
            ...
            pass
    elif repository.data.category == "netdaeperson2":
        try:
            await repository.hacs.hass.services.async_call(
                "hassio", "addon_restart", {"addon": "e466aeb3_netdaeperson2"}
            )
        except Exception:  # pylint: disable=broad-except
            pass
