...
class HacsData(Hacs):
    ...
    def write(self):
        ...
        installed = {}
        for repository_name in self.comperson2.installed:
    ...
    async def restore(self):
        ...
        try:
            ...
            for repository in installed:
                self.comperson2.installed.append(repository)
            ...
            for entry in repositrories:
                ...
                if repo["full_name"] in self.comperson2.installed:
            ...
            for repository in self.repositories:
                if repository.status.installed:
                    ...
                    if (
                        repository.information.full_name not in self.comperson2.installed
                        and repository.information.full_name != "custom-components/hacs"
                    ):
                        self.logger.warning(
                            f"{repository.information.full_name} is not in comperson2.installed"
                        )
                    count_installed_restore += 1
