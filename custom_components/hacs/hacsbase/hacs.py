...
class Hacs(HacsHelpers):
    ...
    async def recurring_tasks_installed(self, notarealarg=None):
        ...
        for repository in self.repositories:
            if (
                ...
                and repository.data.category in self.comperson2.categories
            ):
    ...
    async def recurring_tasks_all(self, notarealarg=None):
        ...
        for repository in self.repositories:
            if repository.data.category in self.comperson2.categories:
                self.queue.add(self.factory.safe_comperson2_update(repository))
    ...
    async def async_load_default_repositories(self):
        ...
        for category in self.comperson2.categories or []:
    ...
    async def async_get_category_repositories(self, category):
        ...
        for repo in repositories:
            ...
            if repository is not None:
                if str(repository.data.id) not in self.comperson2.default:
                    self.comperson2.default.append(str(repository.data.id))
                else:
                    continue
                continue
