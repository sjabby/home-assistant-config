...
def _setup_extra_stores():
    ...
    if "python_script" in hacs.hass.config.components:
        if "python_script" not in hacs.comperson2.categories:
            hacs.comperson2.categories.append("python_script")

    if (
        hacs.hass.services._services.get("frontend", {}).get("reload_themes")
        is not None
    ):
        if "theme" not in hacs.comperson2.categories:
            hacs.comperson2.categories.append("theme")
