        ...
        const.DOMAIN: vol.Schema(
            {
                ...
                vol.Optional("appdaeperson2", default=False): cv.boolean,
...
async def hacs_startup(hacs):
    ...
    if hacs.configuration.appdaeperson2:
        const.ELEMENT_TYPES.append("appdaeperson2")
    ...
    hacs.comperson2.categories = sorted(const.ELEMENT_TYPES)
...
async def test_repositories(hacs):
    ...
    await hacs().register_repository("ludeeus/ad-hacs", "appdaeperson2")
    ...
    await hacs().register_repository("custom-cards/person2ster-card", "plugin")  # root
