...
async def hacs_config(hass, connection, msg):
    ...
    content["categories"] = Hacs().comperson2.categories
...
async def hacs_repositories(hass, connection, msg):
    ...
    for repo in repositories:
        if repo.information.category in Hacs().comperson2.categories:
            data = {
...
async def hacs_repository_data(hass, connection, msg):
    ...
    if action == "add":
        ...
        if repo_id in Hacs().comperson2.skip:
            Hacs().comperson2.skip.remove(repo_id)
