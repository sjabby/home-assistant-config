...
class HacsRepository:
    ...
    def custom(self):
        ...
        if self.data.full_name.split("/")[0] in ["custom-components", "custom-cards"]:
            return False
        if self.data.full_name.lower() in [x.lower() for x in self.hacs.comperson2.default]:
            return False
        if self.data.full_name == "hacs/integration":
            return False
        return True
    ...
    async def comperson2_validate(self):
        ...
        await comperson2_validate(self)
    ...
    async def comperson2_update(self):
        ...
        # Attach repository
        await comperson2_update_data(self)
    ...
    def remove(self):
        ...
        if self.information.uid in self.hacs.comperson2.installed:
            self.hacs.comperson2.installed.remove(self.information.uid)
