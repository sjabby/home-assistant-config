...
class HacsRepository:
    ...
    def custom(self):
        ...
        if self.data.full_name.split("/")[0] in ["custom-components", "custom-cards"]:
            return False
        if str(self.data.id) in [str(x) for x in self.hacs.comperson2.default]:
            return False
    ...
    async def comperson2_validate(self, ignore_issues=False):
        """Comperson2 validation steps of the repository."""
        await comperson2_validate(self, ignore_issues)
    ...
    def remove(self):
        ...
        if self.data.id in self.hacs.comperson2.installed:
            self.hacs.comperson2.installed.remove(self.data.id)
    ...
    async def uninstall(self):
        ...
        if self.data.category == "integration":
            ...
            try:
                ...
                pass
        if self.data.full_name in self.hacs.comperson2.installed:
            self.hacs.comperson2.installed.remove(self.data.full_name)
